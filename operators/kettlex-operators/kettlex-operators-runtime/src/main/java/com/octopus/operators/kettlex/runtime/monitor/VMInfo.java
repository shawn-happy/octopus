...
public class VMInfo {
    ...
    try {
      if (VMInfo.isSunOsMBean(osMXBean)) {
        ...
        if ((curUptime > lastUpTime) && (curProcessTime >= lastProcessCpuTime)) {
          float curDeltaCpu =
              (float) (curProcessTime - lastProcessCpuTime)
      ...
      if (print) {
        log.info(
            processCpuStatus.getDeltaString()
                + processMomoryStatus.getDeltaString()
  ...
  public static long getLongFromOperatingSystem(
    ...
    try {
      ...
      log.info(
          String.format(
              "OperatingSystemMXBean %s failed, Exception = %s ", methodName, e.getMessage()));
    }
