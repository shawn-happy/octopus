...
public class Communication {
  ...
  public synchronized void setState(ExecutionState state, boolean isForce) {
    if (!isForce && this.state.equals(ExecutionState.FAILED)) {
      return;
    }
   ...
   */
  public synchronized ExecutionState mergeStateFrom(final Communication otherComm) {
    ExecutionState retState = this.getState();
    if (otherComm == null) {
      return retState;
    }

    if (this.state == ExecutionState.FAILED
        || otherComm.getState() == ExecutionState.FAILED
        || this.state == ExecutionState.KILLED
        || otherComm.getState() == ExecutionState.KILLED) {
      retState = ExecutionState.FAILED;
    } else if (this.state.isRunning() || otherComm.state.isRunning()) {
      retState = ExecutionState.RUNNING;
    }
  ...
  public synchronized boolean isFinished() {
    return this.state == ExecutionState.SUCCEEDED
